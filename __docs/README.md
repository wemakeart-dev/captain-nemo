...
| --- | --- | --- | --- |
| Web | [`src/web`](../src/web) | [WEB.md](WEB.md) | Hybrids controls, file manager, Import Data modal, chart, `chartStore`, ECharts options, worker RPC from the UI. CSV import is a filesystem path string. Select loads the chart. |
...
| Engine | [`src/engine`](../src/engine) | [ENGINE.md](ENGINE.md) | CSV ingest, SQLite library (`nemo-library.sqlite`), Parquet catalog, paced playback, protobuf WebSocket server. |
...
- CSV, SQLite library, catalog, bars, playback clock, or the Python process → [ENGINE.md](ENGINE.md)
