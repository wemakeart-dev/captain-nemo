...
| --- | --- | --- | --- |
| Web | [`src/web`](../src/web) | [WEB.md](WEB.md) | Hybrids controls and chart, `chartStore`, ECharts options, worker RPC from the UI. CSV import is a filesystem path string. |
...
| Engine | [`src/engine`](../src/engine) | [ENGINE.md](ENGINE.md) | CSV ingest, Parquet catalog, paced playback, protobuf WebSocket server. |
...
- UI, chart, store, or controls → [WEB.md](WEB.md)
- `postMessage` RPC, conflation, or the engine socket in the browser → [WORKER.md](WORKER.md)
- CSV, catalog, bars, playback clock, or the Python process → [ENGINE.md](ENGINE.md)
